...
const YoghurtUpdate = () => {
  ...
  const _get = async () => {
    try {
      const update = await axiosJWT.get(
        `http://localhost:5000/api/product/yoghurt/${id}`,
        {
          headers: {
  ...
  const updateYoghurt = async (e) => {
    e.preventDefault()

    const updateData = new FormData()
    updateData.append('title', title)
    updateData.append('description', desc)
    updateData.append('taste', taste)
    updateData.append('image', img)
    updateData.append('price', price)
    updateData.append('stock', stock)

    try {
      await axiosJWT.put(`http://localhost:5000/api/product/yoghurt/${id}`, updateData, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data"
        }
      })
      navigate("/project/admin/dashboard/myoghurt/")
    } catch (error) {
      if (error.response) {
    ...
    <div className="w-full bg-base-100">
      <div className="container p-4 mx-auto">
        ...
        <div className="flex flex-col w-full gap-1 items-center">
          <form action="" className="border p-1 rounded shadow-lg" onSubmit={updateYoghurt}>
            ...
            <div className="form-control w-full max-w-xs">
                <label className="label">
                  <span className="label-text">Variant Rasa</span>
                  <span className="label-text-alt italic">Choose the taste</span>
                </label>
                <select value={taste} onChange={e => setTaste(e.target.value)}  className="select select-bordered">
                  <option value={""}>Pilih Rasa</option>
                  <option value={"Strowberi"}>Strowberi</option>
